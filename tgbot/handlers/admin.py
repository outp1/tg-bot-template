...
async def admin_advertpanel(
    ...
    action = call.data.split("_")[1]
    if action == "full":
        text = await admin_panel.get_content_advertpanel_full(
            call, advertising_tables, logger
        )
...
async def send_advert_to_user(advert, user_id, bot: Bot):
    if advert["inline_buts"]:
        if type(advert["inline_buts"]) is dict:
            ...
            _type_value = {
                advert["inline_buts"]["_type"]: advert["inline_buts"]["content"]
            }
    ...
    if advert["media_type"] == "media_group":
        ...
        msg = await bot.send_message(
            chat_id=user_id, text=advert["text"], reply_markup=kb
        )
    return msg
...
async def send_advert_preview(
    ...
    if advert["sending_date"] is None:
        ...
        sending_date = datetime.datetime.strftime(
            advert["sending_date"], "%H:%M %Y.%m.%d"
        )
    ...
    if message:
        ...
        try:
            ...
            await advertising_tables.update_advertising(
                advert["advert_id"], "inline_buts", None
            )
            return await message.answer(
                "Ошибка клавиатуры, удаление. Попробуйте ещё раз."
            )
...
async def advert_message_edit_menu(
    ...
    if message:
        if advert_id:
            kbs = AdminPanelKeyboards.get_advert_edit_keybord(
                advert_id, "multiInclose_with_state"
            )
            advert = await advertising_tables.get_advertising("advert_id", advert_id)
            msgs = await send_advert_preview(
                advert, kbs, advertising_tables, bot, message=message, logger=logger
            )
            await state.update_data(msgs=msgs)
        elif data:
            kbs = AdminPanelKeyboards.get_advert_edit_keybord(
                data["advert_id"], "multiInclose_with_state"
            )
            ...
            raise BaseException(
                "Have no advertising id or state data for the function to work"
            )
    else:
        raise BaseException(
            "neither Message nor Call was transmitted"
        )  # TODO: refactor exceptions to external classes
...
async def advert_edit_actions(
    ...
    if action == "media":
        msg = await call.message.answer(
            "Отправьте медиа одним сообщением: ",
            reply_markup=AdminPanelKeyboards.back_to_advert_kb(data[2]),
        )
        ...
        kb.add(
            types.InlineKeyboardButton(
                text="Подтверждаю", callback_data=f"confirmadremove_{advert_id}"
            )
        )
        kb.add(
            types.InlineKeyboardButton(
                text="Отмена", callback_data=f"advert-return_{advert_id}"
            )
        )
        await call.message.answer(
            f"Вы уверены что хотите удалить объявление <code>{advert_id}</code>",
            reply_markup=kb,
        )
    elif action == "kbs":
        advert = await advertising_tables.get_advertising("advert_id", advert_id)
        ...
        kb.add(
            types.InlineKeyboardButton(
                ...
                callback_data=f"advert-kb_clean-add_{advert_id}",
            ),
            types.InlineKeyboardButton(
                text="Добавить", callback_data=f"advert-kb_add_{advert_id}"
            ),
        )
        kb.add(
            types.InlineKeyboardButton(
                text="🔙", callback_data=f"advert-return_{advert_id}"
            )
        )
        if not inline_buts:
            msg = await call.message.answer(
            ...
            buts = [
                f'Текст: <code>{b["text"]}</code> | Тип: <code>{b["_type"]}</code> | Содержимое: {b["content"]}'
                for b in inline_buts
            ]
            text_buts = "\n".join(buts)
        ...
        kb.add(
            types.InlineKeyboardButton(
                text="Подтверждаю", callback_data=f"confirm-advert-sending_{advert_id}"
            )
        )
        text = f"""
...
async def advert_kbtype(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        pass
    msg = await call.message.answer(
        "<b>Введите содержимое кнопки </b>(Ссылка, коллбек, всплывающее окно)"
    )
...
async def advert_kbcontent(
    ...
    if data["_type"] == "popup_message":
        ...
        handler_id = await generate_id_async(
            len_=3, conn_func=advertising_tables.check_handler_id_exists
        )
        content = f'popup-message_{handler_id}_{data["advert_id"]}'
        await advertising_tables.create_handler(
            handler_id, message.text, data["advert_id"]
        )  # TODO
    else:
    ...
    if type(advert["inline_buts"]) is dict:
        ...
        inline_but = json.dumps(inline_but)
    await advertising_tables.update_advertising(
        data["advert_id"], "inline_buts", inline_but
    )
...
async def find_advert_state(
    ...
    try:
        ...
        logger.info(traceback.format_exc())
    await find_advert_content(
        message.text, message, advertising_tables, state, logger, bot
    )
...
async def advert_media(
    ...
    if album:
        ...
        await advertising_tables.update_advertising(
            data["advert_id"], "media", media_group
        )
    else:
        if message.content_type != "text":
            ...
            await advertising_tables.update_advertising(
                data["advert_id"], "media_type", media_type
            )
            await advertising_tables.update_advertising(
                data["advert_id"], "media", file_id
            )
        else:
            return await message.answer(
...
async def newad_header(
    ...
    if len(message.text) > max_header_lenght:
        return await message.answer(
            f"<b>! Заголовок слишком длинный, максимальная длина {max_header_lenght}</b>"
        )
...
async def newad_text(
    ...
    await advertising_tables.add_advertising(
        advert_id=advert_id, advert_header=data["header"], text=data["text"]
    )
    await advert_message_edit_menu(
        advertising_tables, logger=logger, bot=bot, message=message, state=state
    )
...
async def advert_send(
    ...
    try:
        ...
        pass
    advert = await advertising_tables.get_advertising(
    ...
    if action == "!ban":
        if len(data) < 2:
            return await message.reply(
                "Неверный синтаксис. Необходимо передать больше иноформации. /adm для справки."
            )
        await admin_panel.ban_user(
            user=data[1],
            user_tables=user_tables,
            bot=bot,
            modhistory_tables=modhistory_tables,
            num_of=safe_list_get(data, 2, None),
            unit=safe_list_get(data, 3, None),
            message=message,
            logger=logger,
        )  # TODO
    elif action == "!unban":
        ...
        text = " ".join(data[2:])
        await admin_panel.message_to_user(
            user=data[1],
            user_tables=user_tables,
            bot=bot,
            message_text=text,
        ...
        content = await admin_panel.get_user_info(
            data[1], user_tables, bot, message, logger
        )
        ...
        await message.answer(
            "Введите заголовок для рассылки (для удобства отображения):"
        )
        ...
        if len(data) < 2:
            ...
            await find_advert_content(
                data[1], message, advertising_tables, state, logger, bot
            )
...
async def advert_sending_date_actions(
    ...
    if action == "changing":
        msg = await call.message.answer(
            ...
            reply_markup=AdminPanelKeyboards.back_to_advert_kb(advert_id),
        )
        await state.update_data(msg=msg, advert_id=advert_id)
...
async def advert_changing_sending_date(
    ...
    try:
        ...
        block2 = datetime.datetime.now(pytz.timezone("Europe/Moscow")).strftime(
            "%Y.%m.%d"
        )
...
"""
        msg = await message.answer(
            text, reply_markup=AdminPanelKeyboards.back_to_advert_kb(data["advert_id"])
        )
        return await state.update_data(msg=msg)
    # TODO: время записывается на час позже
    logger.info(final_date)
    await advertising_tables.update_advertising(
        data["advert_id"], "sending_date", final_date
    )
    await advertising_tables.update_advertising(
        data["advert_id"], "sending_status", False
    )
    advert = await advertising_tables.get_advertising("advert_id", data["advert_id"])
    logger.info(advert["sending_date"])
    await message.answer(
        "Отправка успешно назначена на <code>" + final_date_str + "</code>"
    )
    await advert_message_edit_menu(
        advertising_tables,
        logger,
...
def register_admin(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        admin_user_actions,
        lambda call: call.data.startswith("adm-user_"),
        state="*",
        is_admin=True,
    )
    ...
    dp.register_message_handler(
        newad_header, state=AdminStates.newad_header, is_admin=True
    )
    dp.register_message_handler(newad_text, state=AdminStates.newad_text, is_admin=True)

    dp.register_callback_query_handler(
        advert_edit_actions, lambda call: call.data.startswith("advertedit_"), state="*"
    )
    dp.register_message_handler(
        advert_media,
    ...
    dp.register_callback_query_handler(
        admin_advertpanel, lambda call: call.data.startswith("advertpanel_")
    )

    dp.register_message_handler(find_advert_state, state=AdminStates.find_advert)

    dp.register_callback_query_handler(
        advert_remove, lambda call: call.data.startswith("confirmadremove_")
    )
    ...
    dp.register_callback_query_handler(
        ...
        lambda call: call.data.startswith("popup-message_"),
        state="*",
    )

    dp.register_callback_query_handler(
        advert_button_add, lambda call: call.data.startswith("advert-kb_")
    )

    dp.register_callback_query_handler(
        advert_send, lambda call: call.data.startswith("confirm-advert-sending_")
    )
    # dp.register_callback_query_handler(check_call)

    dp.register_callback_query_handler(
        advert_sending_date_actions,
        lambda call: call.data.startswith("advert-send-date_"),
    )

    dp.register_message_handler(
        advert_changing_sending_date, state=AdminStates.advert_sending_date
    )
