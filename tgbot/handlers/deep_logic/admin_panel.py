...
async def take_stats_content(user_tables: UserTables, logger: logging.Logger = logging,
    ...
    statistic = statisticer.BotStatistic(users_list, reg_date_index=reg_date_index, logger=logger)
...
async def ban_user(user: str, user_tables: UserTables, bot: Bot, message: types.Message, 
        modhistory_tables: ModeratingHistoryTables, num_of: str = None, 
    ...
    if status == 'Продление':
        await user_tables.add_user_history(user, 
        ...
        await bot.send_message(user, f'Вам продлили блокировку до <code>{unbanned_date_text_format}</code>')
    elif status == 'Новая':
        await user_tables.add_user_history(user, 
        ...
        await message.answer(
                ...
                disable_web_page_preview=True)
        await bot.send_message(user, f'Вам дали блокировку до <code>{unbanned_date_text_format}</code>')
    action_to_modhistory = f'Блокировка пользователя {user_profile.mention} до {unbanned_date_text_format}'
    await modhistory_tables.add_entry_to_history(str(message.from_user.id), from_user['role'], 
            action_to_modhistory, date=now)


async def unban_user(user: str, user_tables: UserTables, bot: Bot, message: types.Message, 
        modhistory_tables: ModeratingHistoryTables, logger: logging.Logger = logging):
    now = datetime.datetime.now(tz=pytz.timezone('Europe/Moscow'))
    now_str = now.strftime('%Y-%m-%d %H:%M')
    user_info = await user_tables.take_user('user_id', user)
    user_profile = await bot.get_chat(user)
    ...
    if user_info['unbanned_date'] <= now:
        return await message.answer(
                ...
                disable_web_page_preview=True)
    await user_tables.add_user_history(user, f'<code>{now_str} | {message.from_user.mention} | Ручное снятие блокировки</code>')
    action_to_modhistory = f'Ручное снятие блокировки пользователю {user_profile.mention}'
    await modhistory_tables.add_entry_to_history(str(message.from_user.id), 
            from_user['role'], action_to_modhistory, date=now)
    ...
    await message.answer('<a href="t.me/{user_profile.mention[1:]}">Пользователь</a> разблокирован',
            disable_web_page_preview=True)
    await bot.send_message(chat_id=user, text=f'<a href="t.me/{message.from_user.mention[1:]}">Администратор</a> снял вашу блокировку',
            disable_web_page_preview=True)

async def get_user_info(user: str, user_tables: UserTables, bot: Bot, 
        message: types.Message, logger: logging.Logger = logging):
    user_info = await user_tables.take_user('user_id', user)
    message_text = await format_user_info_to_text(user_info) 
    return message_text 

async def message_to_user(user: str, user_tables: UserTables, bot: Bot, message_text: str, 
        message: types.Message, logger: logging.Logger = logging):
    if not message_text:
...
async def get_content_advertpanel_full(call: types.CallbackQuery, advertising_tables: AdvertisingTables, logger: logging.Logger):
    ...
    for ad in adverts:
        ...
        if ad['sending_status'] is False:
            ...
            sending_status = '✅'
        ads_list.append(f"<code>{ad['advert_id']} | {ad['advert_header']} | {sending_date} | {sending_status}</code>")
    ads_text = '\n'.join(ads_list)
    text = f"""
