...
async def multi_inclose_with_state(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        for msg in data['msgs']:
            try:
                ...
                try:
                    for photo in msg:
                        await bot.delete_message(chat_id=call.message.chat.id, message_id=photo.message_id)
                except:
...
def register_misc(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(multi_inclose_with_state, text='multiInclose_with_state', state='*')
