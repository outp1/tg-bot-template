...
class BotStatistic:
    ...
    async def sorted_users_strings(
        self, str_format: str, num_of_lines: int = 1, sort_index: Union[str, int] = None
    ):
    ...
    async def list_of_the_best(
        ...
        *sort_indexes,
        ...
        sorted_users = sorted(
            self.users_list,
            key=lambda x: list(x[i] for i in sort_indexes),
            reverse=reverse,
        )
        users_list = []
        i = 0
