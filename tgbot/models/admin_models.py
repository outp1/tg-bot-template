...
class ModeratingHistoryTables(DatabaseConnection):
    ...
    async def add_entry_to_history(self, user_id, role, action, date):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    "INSERT INTO moderating_history (user_id, role, action, date) VALUES (%s, %s, %s, %s)",
                    (user_id, role, action, date),
                )
...
class AdvertisingTables(DatabaseConnection):
    ...
    async def update_advertising(self, advert_id, key, value):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    f"UPDATE advertising SET {key} = %s WHERE advert_id = %s",
                    (value, advert_id),
                )
    ...
    async def check_for_id(self, _id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    f"SELECT * FROM advertising WHERE advert_id = %s", (_id,)
                )
                return cursor.fetchone()
    ...
    async def remove_advertising(self, advert_id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    f"DELETE FROM advertising WHERE advert_id = %s", (advert_id,)
                )
    ...
    async def take_handler(self, handler_id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    "SELECT * FROM advertising_handlers WHERE handler_id = %s",
                    (handler_id,),
                )
                return cursor.fetchone()
    ...
    async def create_handler(self, handler_id, message, advert_id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    "INSERT INTO advertising_handlers (handler_id, message, advert_id) VALUES (%s, %s, %s)",
    ...
    async def remove_handlers(self, advert_id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    "DELETE FROM advertising_handlers WHERE handler_id = %s",
                    (handler_id,),
                )
    ...
    async def check_handler_id_exists(self, handler_id):
        with self.sqlighter as connection:
            ...
            with connection:
                cursor.execute(
                    f"SELECT * FROM advertising_handlers WHERE handler_id = %s",
                    (handler_id,),
                )
                return cursor.fetchone()
